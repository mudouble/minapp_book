...
Page({
      ...
      detail(e) {
            //let that = this;
            console.log("idkkkkk",e.currentTarget.dataset.id)
            wx.navigateTo({
                  url: '/pages/trade/details/details?id=' + e.currentTarget.dataset.id,
            })
      },
      //搜索结果
      search(n) {
            ...
            db.collection('goods').where({
                  status: 0,
                 //createTime: _.gt(new Date().getTime()),
                  title: db.RegExp({
                        regexp: '.*' + key + '.*',
                        options: 'i',
                  })
      ...
      more() {
            ...
            }).orderBy('createTime', 'desc').skip(page * 20).limit(20).get({
                  success: function (res) {
